...
현재 권장 운영 경로는 Vercel과 GitHub가 프론트/API/자동화를 맡고, 별도 추론 서버가 Qwen + LoRA 직접 서빙을 맡는 구조입니다. Groq는 선택적 폴백으로 남길 수 있지만, custom provider와 local registry만으로도 운영 경로를 구성할 수 있습니다. 학습 구조는 완전 자동 온라인 재학습 루프가 아니라, 토론 데이터를 축적한 뒤 reward_events와 preference_pairs를 만들고, readiness를 평가한 후 배치 스크립트로 export, SFT, DPO를 수행하는 형태입니다.
