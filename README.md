...
2. 필요하면 .env에 VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY, GROQ_API_KEY를 설정합니다.
2-0. 모델 API 키는 브라우저에 노출되지 않도록 VITE_ 접두사를 붙이지 않습니다.
...
재학습 readiness와 dataset export까지 정상 동작하려면 GitHub Actions Secrets 에 SUPABASE_SERVICE_ROLE_KEY 를 추가해야 합니다. anon 키만으로는 RLS 이후 보호된 테이블을 읽지 못해 readiness/export 가 401 로 실패합니다. 실제 self-hosted 학습까지 돌리려면 runner 환경에 Python, torch 계열 패키지, 그리고 필요하면 Ollama가 추가로 준비돼 있어야 합니다.
...
reward-learning을 기존 프로젝트에 추가하려면 db/supabase_add_reward_learning_tables.sql 을 적용하세요. 이 단계는 선택 사항이고, 적용 전에도 기본 운영은 가능합니다. 이 SQL 적용 경로는 Supabase service role 이 아니라 direct Postgres 연결을 사용하므로 SUPABASE_DB_URL 또는 SUPABASE_DB_PASSWORD 가 준비돼 있어야 합니다. 이미 DB 접속 정보가 있다면 PowerShell에서 다음처럼 실행할 수 있습니다.

- $env:SUPABASE_SQL_FILE='db/supabase_add_reward_learning_tables.sql'; npm run apply-rls

가상창고를 기존 프로젝트에 추가하려면 db/supabase_add_virtual_warehouse.sql 을 적용하세요. debate-archives 와 training-datasets 버킷, debate_archives 와 model registry 테이블이 함께 생성됩니다. 이 경로도 direct Postgres 연결을 사용하므로 SUPABASE_DB_URL 또는 SUPABASE_DB_PASSWORD 가 필요합니다.
