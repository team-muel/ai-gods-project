...
```bash
curl "http://127.0.0.1:8787/jobs/<job_id>?tail=80"
```
...
- https://gpu.example.com/train
- https://gpu.example.com/healthz

가장 단순한 방법은 Caddy 나 Nginx 로 127.0.0.1:8787 을 reverse proxy 하는 것이다.

## 7. GitHub 에 넣을 값

Repository secrets:

- REMOTE_TRAINING_WEBHOOK_URL=https://gpu.example.com/train
