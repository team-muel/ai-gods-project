...
GitHub secret `GOOGLE_SERVICE_ACCOUNT_JSON` 이 Compute Engine 생성 권한을 가진다면, 수동 생성 대신 `.github/workflows/provision-gcp-gpu-remote-training.yml` workflow_dispatch 로 GPU VM 생성과 웹훅 bootstrap 을 자동화할 수 있다.
...
REMOTE_TRAINING_REPO_DIR=/opt/muel/ai-gods-project-remote
REMOTE_TRAINING_JOB_ROOT=/opt/muel/ai-gods-remote-jobs
...
REMOTE_TRAINING_REQUIRE_GPU=true
...
정상적인 GPU VM 이면 health 응답의 gpu.available 이 true 여야 한다. false 이면 /train 은 503 으로 거부된다.
...
- 현재 webhook 은 GPU 가 없는 VM 에서는 /train 요청을 503 gpu_unavailable 로 거부한다. healthz 의 gpu 필드를 먼저 확인한다.
...
- 장기 운영 시 /opt/muel/ai-gods-remote-jobs 아래 오래된 job 로그를 주기적으로 지운다.
